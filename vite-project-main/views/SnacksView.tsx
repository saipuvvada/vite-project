...
import { ChevronLeft, Info, Plus, Minus, Pizza } from "lucide-react";
import { motion } from "framer-motion";

const SNACKS_PHONE = "8639186035";

const SnacksView: React.FC = () => {
  const navigate = useNavigate();
  const { cart, addToCart, removeFromCart, setRestaurantPhone } = useCart();

  const [items, setItems] = useState<MenuItem[]>([]);
  const [snacksId, setSnacksId] = useState("snacks");
  ...
  useEffect(() => {
    Promise.all([
      ...
      const snacksRes = resData.find((r: any) => r.name === "Snacks");
      if (snacksRes) {
        setSnacksId(snacksRes._id);
        setRestaurantPhone(snacksRes.phone || SNACKS_PHONE);
        
        api.getMenuByRestaurant(snacksRes._id).then(menuData => {
  ...
  return (
    <div className="p-4 pb-48 max-w-2xl mx-auto">
      ...
      <div className="mb-10">
        <h1 className="text-4xl font-extrabold tracking-tight mb-2">Explore Snacks</h1>
        <p className="text-slate-500 dark:text-slate-400 font-medium">Crunchy, hot, and tasty fast food</p>
      </div>
        ...
        <div className="relative z-10">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-white/50 dark:bg-black/20 p-4 rounded-2xl">
              ...
              <div className="grid grid-cols-2 gap-y-2 gap-x-6 text-sm font-bold text-emerald-900 dark:text-emerald-100">
      ...
      {loading ? (
        ...
        <div className="text-center py-10">
          <p className="text-slate-500 font-bold">No items found. Please check back later!</p>
        </div>
      ) : (
                      ...
                      <div className="relative">
                        {qty > 0 ? (
                          <div className="flex items-center gap-4 bg-primary text-white p-1.5 rounded-2xl shadow-lg shadow-primary/30">
                            ...
                            <button 
                              onClick={() => addToCart(item, snacksId)}
                              className="w-8 h-8 flex items-center justify-center rounded-xl hover:bg-white/20 transition-colors"
                            >
                              <Plus size={18} />
                            </button>
      ...
      <div className="mt-12 p-6 rounded-3xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-500 text-sm font-medium">
        <div className="flex gap-3">
          ...
          <p>Snacks are prepared fresh and served piping hot. Please consult with our delivery executive for preparation timings during peak lunch hours.</p>
        </div>
      </div>
    </div>
  );
