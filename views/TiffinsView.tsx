...
import { TIFFIN_MENU_ITEMS } from "../constants";
import { MenuItem } from "../types";

type Props = { onBack: () => void };

const TIFFIN_PHONE = "8639186035"; // ✅ change to your WhatsApp number

const TiffinsView: React.FC<Props> = ({ onBack }) => {
  const { cart, addToCart, removeFromCart, setRestaurantPhone } = useCart();

  useEffect(() => {
    setRestaurantPhone(TIFFIN_PHONE);
  }, [setRestaurantPhone]);

  const grouped = useMemo(() => {
    const g: Record<string, MenuItem[]> = {};
    TIFFIN_MENU_ITEMS.forEach((it) => {
  ...
  return (
    <div className="p-4 pb-32 max-w-2xl mx-auto">
      ...
      <h1 className="text-3xl font-black mb-3">Explore Tiffins</h1>
                ...
                >
                  ...
                  {qty > 0 ? (
                    <div className="flex items-center gap-3 bg-primary text-white p-2 rounded-xl">
                      ...
                      <button onClick={() => addToCart(item, "tiffins")}>+</button>
                    </div>
                  ) : (
                    <button
                      onClick={() => addToCart(item, "tiffins")}
